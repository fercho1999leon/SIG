...
@php
use App\Permiso;
$permiso = Permiso::desbloqueo('financiero.index');
@endphp
    <div id="page-wrapper" class="gray-bg dashbard-1">
        @include('layouts.nav_bar_top')
        <div class="row wrapper white-bg ">
			<div class="col-lg-12 titulo-separacion">
				<h2 class="title-page">Fichas del Personal Secretaría</h2>
			</div>
		</div>
		<div class="wrapper wrapper-content">
			@if($permiso ==null || ($permiso != null && $permiso->editar == 1))
			<div class="agregarSeccionCont">
				<a href="{{route('secretaria.create')}}">
					<button class="btn dirConfiguraciones__instituto--agregarInfo" >Agregar Personal Secretaría</button>
				</a>
			</div>
			@endif
					...
					<div class="fAdministrativo">
						<div class="a-personal-administrativo__card relative">
							...
							<div class="text-center a-personal-administrativo__icons">
								<div>
									<span>
										<a data-route="{{route('secretaria.show', $user)}}" class="icon__ver">
											<i class="fa fa-eye"></i>
										</a>
									</span>
									@if($permiso ==null || ($permiso != null && $permiso->editar == 1))
									<span>
										<a href="{{route('secretaria.edit', $user)}}" class="icon__editar">
									...
									<span>
										<form action="{{route('secretaria.destroy', $user)}}" method="post" class="icon__eliminar-form form-delete" onclick="return confirm('¿Seguro desea eliminar a este usuario?')" >
	...
	{{-- modal ver secretaría --}}
	<div class="modal fade" id="detalleSecretaria" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	</div>
@endsection
@section('scripts')
<script>
	$('.icon__ver').click(function() {
		$.ajax({
			...
			success: function (response) {
				$('#detalleSecretaria').html(response)
				$('#detalleSecretaria').modal('show')
			}, error: function() {
