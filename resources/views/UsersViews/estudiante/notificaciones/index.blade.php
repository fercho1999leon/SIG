                            ...
                            @foreach($messages as $message)
								...
								@endif
									<td class="mail-contact" width="200">
										<a href="{{ route('notificacionesVerEstudiante',['id' => $message->id]) }}">
									...
									<td class="mail-subject">
										<a href="{{ route('notificacionesVerEstudiante',['id' => $message->id]) }}">{{ $message->razon }}</a>
									</td>
									<td class="text-right mail-date" width="100">{{ date('d-m-Y', strtotime($message->created_at))  }}
									</td>
									<td width="5">
										@php
											$eliminarMensaje = App\ConfiguracionSistema::eliminarMensajesEstudiantes();
