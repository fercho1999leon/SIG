...
<body>
	...
	<table class="table whitespace-no">
		...
		@foreach($students as $student)
			@php
			$remedial[$student->id] = false;
			$supletorio[$student->id] = false;
					...
					@foreach($matters->where('principal', 1) as $matter)
						...
						@endphp
							@if($notas_materia!=null)
								@php
								if($notas_materia->supletorio<7 && $notas_materia->supletorio > 0 ){
									$remedial[$student->id] = true;
								}elseif($notas_materia->supletorio == 0){
									$supletorio[$student->id] = true;
								}else{
									$aprobado[$student->id] =  true;
								}
								@endphp
							<td class="text-center" 
                                @if($notas_materia->promedio < 7 && $notasMenores == "1" && $notas_materia->promedio !=0)
							        style="color:red;" 
                                @endif>{{bcdiv($notas_materia->promedio, '1', 2)}}
                            </td>
							<td class="text-center" 
                                @if($notas_materia->supletorio < 7 && $notasMenores == "1" && $notas_materia->supletorio !=0)
                                    style="color:red;" 
                                @endif>{{bcdiv($notas_materia->supletorio, '1', 2)}}
                            </td>
							<td class="text-center" 
                                @if($notas_materia->promedioFinal < 7 && $notasMenores == "1" && $notas_materia->promedioFinal !=0)
                                    style="color:red;" 
                                    @endif>{{bcdiv($notas_materia->promedioFinal, '1', 2)}}
                                </td>
							@else
		...
		@foreach($students as $student)
			...
			@if($notas->count()>0)
				<tr>
					...
					@foreach($matters->where('principal', 0) as $matter)
						...
						@endphp
							@if($notas_materia!=null)
								...
								<td class="text-center" @if($notas_materia->supletorio < 7 && $notasMenores == "1" && $notas_materia->supletorio !=0)style="color:red;" @endif>{{bcdiv($notas_materia->supletorio, '1', 2)}}</td>
					...
					@endforeach
					<td>
					@if($remedial[$student->id])
						REMEDIAL
					@elseif($supletorio[$student->id])
					SUPLETORIO
					@else
					APROBADO
					@endif</td>
