					...
					@foreach($matters->where('principal', 1) as $matter)
						...
						@endphp
							@if($notas_materia!=null)
								@php
								if($notas_materia->remedial<7 && $notas_materia->remedial > 0 ){
									$gracia[$student->id] = true;
								}elseif($notas_materia->remedial == 0){
							...
							<td class="text-center" @if($notas_materia->remedial < 7 && $notasMenores == "1" && $notas_materia->remedial !=0)style="color:red;" @endif>{{bcdiv($notas_materia->remedial, '1', 2)}}</td>
		...
		@foreach($students as $student)
			...
			@if($notas->count()>0)
				<tr>
					...
					@endforeach
					<td>
					@if($gracia[$student->id])
						GRACIA
					@elseif($remedial[$student->id])
					REMEDIAL
					@else
					APROBADO
					@endif</td>
