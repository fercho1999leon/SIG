	...
	<main>
		...
		<table class="table">
			...
			@foreach($estudiantes as $student)
				@php
				$equiv = "";
					if($student->promedioEstudiante >9){
						$equiv = "DAR";
					}else if($student->promedioEstudiante >=7 && $student->promedioEstudiante <9){
						$equiv = "AAR";
					}else if($student->promedioEstudiante >=4 && $student->promedioEstudiante <7){
						$equiv = "PAAR";
					}else {
						$equiv = "NAAR";
					}
				...
				<tr>
					...
					<td class="text-center">{{$student->promedioEstudiante !=0 ? bcdiv($student->promedioEstudiante, '1', 2) :'' }}</td>
					<td class="text-center uppercase">{{ $equiv }}</td>
					<td class="text-center uppercase">
						@foreach ($comportamientos->where('idStudent', $student->estudianteId) as $comportamiento)
							{{$comportamiento->nota}}
						@endforeach
					</td>
				</tr>
