...
@section('content')
	<table class="table-no-border">
		...
		<tr>
			...
			<td class="text-right">{{$abono->created_at}}</td>
		</tr>
		<tr>
		...
		<td class="text-right"><h3 class="m-0 bold">${{ $abono->cantidad }}</h3></td>
		</tr>
		<tr>
			<td colspan="2"><br></td>
		</tr>
		...
		@foreach($factura->facturaDetalle as $fd)
			@php
				...
				$valor_a_pagar = bcdiv(App\pagoEstudianteDetalle::descuento($pd), '1', 2);
			@endphp
			<tr>
				<td>Descripción</td>
			...
			<tr>
				<td>Total a pagar</td>
				<td class="text-right">${{ $valor_a_pagar }}</td>
			</tr>
		@endforeach
		<tr>
			<td>Total Abonado</td>
		...
		<tr>
			...
			<td class="text-right">${{$valor_a_pagar - $abono->cantidad}}</td>
		</tr>
	</table>
@endsection