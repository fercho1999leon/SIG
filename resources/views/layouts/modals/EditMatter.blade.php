                ...
                <div class="tab-content">
                    <div id="materia" class="tab-pane fade in active">
                        ...
                        <div class="agregarCurso--info">
                            ...
                            <input id="subjAbrv" type="text" name="nombre_abreviado"
                                value="{{ $matter->nombre_abreviado }}" class="form-control">
                        </div>
                        <div class="agregarCurso--info">
                                        ...
                                        <select class="form-control" id="area" name="area">
                                            ...
                                            @foreach ($areas->where('seccion', 'EI') as $area)
                                                <option value="{{ $area->id }}"
                                                    {{ $matter->idArea == $area->id ? 'selected' : '' }}>
                                                    {{ $area->nombre }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="agregarCurso__labels">
                                <div>
                                    <input type="radio" name="visible" value="true"
                                        @if ($matter->visible) checked @endif>
                        ...
                        <div class="agregarCurso--aula">
                            ...
                            <div class="agregarCurso__labels">
                                <div>
                                    <input type="radio" name="principal" value="true"
                                        @if ($matter->principal) checked @endif>
                                    <label for="Si1">Si</label>
                                </div>
                                <div>
                                    <input type="radio" name="principal" value="false"
                                        @if (!$matter->principal) checked @endif>
                                    <label for="No1">No</label>
                                </div>
                            </div>
                        ...
                        <div class="agregarCurso--aula">
                            ...
                            <div class="agregarCurso__labels">
                                <div>
                                    <input type="checkbox" name="t_calif" id="t_calif_{{ $matter->id }}"
                                        {{ $matter->idEstructura != null ? 'checked' : '' }}
                                        onclick="cualitativa({{ $matter->id }})">
                                    <label for="Si1">Si&nbsp;&nbsp;&nbsp;</label>
                                    <div id="ver_menu_cualitativo_{{ $matter->id }}"
                                        style="display: {{ $matter->idEstructura != null ? 'block' : 'none' }}">
                                        <select name="idEstructura" id="idEstructura{{ $matter->id }}"
                                            class="form-control">
                                            <option value="">Sin asignación...</option>
                                            @foreach ($cualitativas as $cualita)
                                                <option value="{{ $cualita->id }}"
                                                    {{ $matter->idEstructura == $cualita->id ? 'selected' : '' }}>
                                                    {{ $cualita->nombre }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="idDocente" name="idDocente">
                                    ...
                                    @foreach ($docentes as $docente)
                                        <option value="{{ $docente->userid }}"
                                            {{ $matter->idDocente == $docente->userid ? 'selected' : '' }}>
                                            {{ $docente->apellidos }} {{ $docente->nombres }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
