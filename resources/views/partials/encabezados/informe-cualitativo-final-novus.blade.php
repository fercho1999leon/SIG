		...
		<th style="vertical-align:top;" class="no-border" width="5%">
			<div class="header__logo" style="float: left">
				<img
					...
					@else
						src="{{ secure_asset('/storage/logo/'.DB::table('institution')->where('id', '1')->first()->logo) }}"  width="60"
					@endif
				alt="" width="70">
			</div>
		</th>
		<th class="no-border" width="60%">
			<div class="header__info text-left">
				...
				@else
				    ...
				    @endphp
					<h3 class="uppercase h2">BOLETÍN DE CALIFICACIONES</h3>
					<h3 class="uppercase h2">{{$tipo ?? ''}}</h3>
					<h3 class="uppercase h2">{{$educacion}}</h3>
					<h3 class="uppercase h2">{{$nombre}}</h3>
                    <h3 class="uppercase h2">AÑO LECTIVO: {{$periodo}} </h3>
                @endif
            </div>
		</th>
	</tr>
